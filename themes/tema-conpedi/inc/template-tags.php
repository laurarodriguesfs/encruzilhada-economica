...
if ( ! function_exists( 'newspack_posted_by' ) ) :
	...
	function newspack_posted_by() {
		...
		if ( function_exists( 'coauthors_posts_links' ) && ! empty( get_coauthors() ) ) : // phpcs:ignore PHPCompatibility.LanguageConstructs.NewEmptyNonVariable.Found
			...
			<span class="byline">
				<span><?php echo esc_html__('newspack' ); ?></span>
				<?php
				foreach ( $authors as $author ) {
...
if ( ! function_exists( 'newspack_author_fixed_links' ) ) :
	/**
	 * Prints list of fixed links for the current author.
	 */
	function newspack_author_fixed_links( $author_id, $size = 24 ) {
		// Get list of available fixed profiles.
		$fixed_profiles = array(
			'facebook',
			'twitter',
			'instagram',
			'linkedin',
		...
		foreach ( $fixed_profiles as $profile ) {
			if ( '' !== get_the_author_meta( $profile, $author_id ) ) {
				if ( 'twitter' === $profile ) {
					$links .= '<li class="twitter"><a href="https://twitter.com/' . esc_attr( get_the_author_meta( $profile, $author_id ) ) . '" target="_blank">' . newspack_get_fixed_icon_svg( $profile, $size, $profile ) . '</a></li>';
				} else {
					$links .= '<li class="' . esc_attr( $profile ) . '"><a href="' . esc_url( get_the_author_meta( $profile, $author_id ) ) . '" target="_blank">' . newspack_get_fixed_icon_svg( $profile, $size, $profile ) . '</a></li>';
				}
			}
		}

		if ( '' !== $links && true === get_theme_mod( 'show_author_fixed', false ) ) {
			echo '<ul class="author-fixed-links">' . wp_kses( $links, $allowed_html ) . '</ul>';
		}
	}
endif;
...
/**
 * Displays fixed links menu for the header; includes AMP toolbar and toolbar-target attributes.
 */
function newspack_fixed_menu_header() {
	...
	$toolbar_attributes = 'toolbar="(min-width: 767px)" toolbar-target="fixed-nav-contain"';
...
/**
 * Displays fixed links menu for the footer; without AMP-related attributes, to prevent duplication errors.
 */
function newspack_fixed_menu_footer() {
	if ( ! has_nav_menu( 'fixed' ) ) {
		return;
	}
	?>
