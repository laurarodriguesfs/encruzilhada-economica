...
?>
	<section id="primary" class="content-area">
		...
		<nav class="agenda-filtros">
			<span>Filtre por:</span>
			
			<a href="?tempo=passados">Eventos passados</a>

			<a href="<?php echo get_post_type_archive_link('item_da_agenda'); ?>">Próximos eventos</a>
			
			<select onchange="location = this.value;">
				<option value="<?php echo get_post_type_archive_link('item_da_agenda'); ?>">Por mês</option>
				<?php
				for ($m=1; $m<=12; $m++) {
					$valor = str_pad($m, 2, '0', STR_PAD_LEFT);
					$nome = date_i18n('F', mktime(0, 0, 0, $m, 1));
					// A URL aqui contém apenas o mês, removendo o 'tempo'
					echo '<option value="?mes_filtro='.$valor.'" '.(isset($_GET['mes_filtro']) && $_GET['mes_filtro'] == $valor ? 'selected' : '').'>'.ucfirst($nome).'</option>';
				}
				?>
			</select>
		</nav>
		...
		<main id="main" class="site-main">
			...
			<?php
			// 1. Identifica qual o CPT da archive atual (ex: 'projetos', 'servicos', etc)
			$cpt_atual = get_query_var('post_type');

			// Se for uma archive de categoria ou tag comum, o post_type pode vir vazio ou como array
			if ( is_array($cpt_atual) ) {
				$cpt_atual = $cpt_atual[0];
			}

			// 2. Busca o cabeçalho que tem o slug IGUAL ao nome do CPT
			$header_query = new WP_Query([
				'post_type'      => 'archive_header',
				'name'           => $cpt_atual, // Busca pelo slug do post
				'posts_per_page' => 1,
				'post_status'    => 'publish'
			]);
			?>

			<section class="archive-custom-header">
				<?php
        ...
        global $wp_query;
        // Adiciona o botão "Carregar Mais" apenas se houver mais posts
